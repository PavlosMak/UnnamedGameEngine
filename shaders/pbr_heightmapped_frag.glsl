...
layout(location = 7) uniform sampler2D albedoMap;
layout(location = 8) uniform sampler2D roughnessMap;
layout(location = 9) uniform sampler2D metallicMap;
layout(location = 10) uniform sampler2D ambientMap;

layout(location = 11) uniform vec3 lightPos[NUM_OF_LIGHTS];
layout(location = 11 + NUM_OF_LIGHTS) uniform vec3 lightColor[NUM_OF_LIGHTS];

layout(location = 11 + 2*NUM_OF_LIGHTS) uniform sampler2D shadowMaps[NUM_OF_LIGHTS];
layout(location = 11 + 3*NUM_OF_LIGHTS) uniform mat4 lightMVPs[NUM_OF_LIGHTS];
layout(location = 11 + 4*NUM_OF_LIGHTS) uniform bool isConeLight[NUM_OF_LIGHTS];
...
void main() {
    vec3 normal = fragNormal;//normalize(vec3(texture(normalMap, fragTexCoord)));
