...
Texture::Texture(std::filesystem::path filePath, GLuint clampMode) {
    ...
    glTextureSubImage2D(m_texture, 0, 0, 0, cpuTexture.width, cpuTexture.height, GL_RGB, GL_FLOAT,
                        cpuTexture.pixels.data());

    // Set behavior for when texture coordinates are outside the [0, 1] range.
    glTextureParameteri(m_texture, GL_TEXTURE_WRAP_S, clampMode);
    glTextureParameteri(m_texture, GL_TEXTURE_WRAP_T, clampMode);
